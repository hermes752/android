...
public class EarthQuakesDB {
    ...
    /*private  String[] result_columns = new String[] {
            EarthQuakeProvider.result_columns.ID_COLUMN_NAME, EarthQuakeProvider.result_columns.MAG_COLUMN_NAME,
            EarthQuakeProvider.result_columns.PLACE_COLUMN_NAME, EarthQuakeProvider.result_columns.TIME_COLUMN_NAME,
            EarthQuakeProvider.result_columns.URL_COLUMN_NAME, EarthQuakeProvider.result_columns.LAT_COLUMN_NAME,
            EarthQuakeProvider.result_columns.LONG_COLUMN_NAME, EarthQuakeProvider.result_columns.DEPTH_COLUMN_NAME};*/
   ...
   private ArrayList<EarthQuake> ConvertirConsultaTipoEarthQuake (Cursor cursor){
        ...
        if (cursor != null && cursor.getCount() > 0){
            //cursor.moveToFirst();
            HashMap<String, Integer> colNameIndex = getColumnsIndex(EarthQuakeProvider.result_columns, cursor);
    ...
    public EarthQuake GetEarthQuake (String idEarthQuake){
        ...
        if (consulta!= null && consulta.getCount() > 0){
            HashMap<String, Integer> colum_index = getColumnsIndex(EarthQuakeProvider.result_columns, consulta);
    ...
    public List<EarthQuake> GetEarthQuakesFilerByMag(Double minMag){
        ...
        contentResolver.query(EarthQuakeProvider.CONTENT_URI,EarthQuakeProvider.result_columns,null,null,null);
        return null; //ConvertirConsultaTipoEarthQuake(getEarthQuakesMinMag(minMag, EarthQuakeOpenHelper.ORDER_BY_MAG, -1));
    }


    private Cursor getEarthQuakes (){
        return null; // db.query(EarthQuakeOpenHelper.DATABASE_TABLE, result_columns, null, null, null, null, null);
    }
    ...
    private Cursor getEarthQuakesMinMag(Double minMag, int orderBy, int group){
        ...
        String[] whereArgs;
        if (minMag != null) {
            where = EarthQuakeProvider.MAG_COLUMN_NAME + " > ?";
            String[] wArgs = {
        ...
        ContentResolver contentResolver=context.getContentResolver();
        contentResolver.query(EarthQuakeProvider.CONTENT_URI,EarthQuakeProvider.result_columns,where,whereArgs,null);
        /*
        ...
        return null; //db.query(EarthQuakeOpenHelper.DATABASE_TABLE, result_columns, where, whereArgs, groupBy, having, order);
    }
 ...
 /*       ContentValues newEQBD = new ContentValues();
    ...
    private EarthQuake CursorToEarthQuake (Cursor cursor, HashMap<String, Integer> colNameIndex){
        ...
        if (cursor != null){
            eq = new EarthQuake();
            eq.setId(cursor.getString(colNameIndex.get(EarthQuakeProvider.result_columns[0])));
            eq.setMagnitude(cursor.getDouble(colNameIndex.get(EarthQuakeProvider.result_columns[1])));
            eq.setPlace(cursor.getString(colNameIndex.get(EarthQuakeProvider.result_columns[2])));
            eq.setTime(cursor.getLong(colNameIndex.get(EarthQuakeProvider.result_columns[3])));
            eq.setUrl(cursor.getString(colNameIndex.get(EarthQuakeProvider.result_columns[4])));

            Coordinate coords = new Coordinate(
                    cursor.getDouble(colNameIndex.get(EarthQuakeProvider.result_columns[5])),
                    cursor.getDouble(colNameIndex.get(EarthQuakeProvider.result_columns[6])),
                    cursor.getDouble(colNameIndex.get(EarthQuakeProvider.result_columns[7]))
            );
            eq.setCoords(coords);
        }
        return eq;
